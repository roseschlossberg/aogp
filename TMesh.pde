...
class TMesh extends NodeGeom {
  ...
  void drawContents() {
    if( mUseTexture ) {
      // Vert's mUV is stored in a normalized {0.0 .. 1.0} range
    ...
    if( mDrawMode == TRISTRIP_MODE ) {
      ...
      for(int i = 0; i < tsCount; i++) {
        ...
        if( mUseTexture ) {
         texture(mTexture); 
          // Set the vertex position (with texture UV)
        vertex( cVert.mPosition.x, cVert.mPosition.y, cVert.mPosition.z, cVert.mUV.x, cVert.mUV.y );
        }
        else {
...
//    endShape();
  ...
  void setTexture(GLTexture iTexture) {
    mTextureGL  = iTexture;
    useTexture(true);
  }
   void setTexture(GSMovie iTexture) {
    pleaseGod  = iTexture;
    useTexture(true);
  }
}
